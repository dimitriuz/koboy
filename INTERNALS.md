...
half is the decisions that look wrong from outside and are not, because the
hardware said so — the ones a newcomer would "fix" into a bug.

Everything measured was measured on a Kobo Libra 2 — Mark 9, single-core
Cortex-A9 with NEON, 1264×1680 at 300 dpi, glibc 2.19 — unless the text says
otherwise. `TESTED.md` has the raw sessions; `docs/FOLLOWUPS.md` has what is
open, and the `#N` references point into it. Its numbers are never reused, so a
`#N` not in the body is in that file's retired table with a note.
...
The rule that makes it work: **everything above the seam is tested on the dev
host.** Pixels arrive at `blit_gray8` as gray8 at final scale; input arrives
already normalised to libretro joypad bits. `main.c` is byte for byte the same
file on a desktop and on the device.
...
- **`wfm_fast_name` reads back off the platform, not the config.** A line
  reporting what `config.c` parsed proves `config.c`; this fails if the setting
  never reached the backend, and on the device it reports the *real mapped*
  waveform — a DU4 request on a panel without the eclipse quirk maps to A2, and
  this is what says so.
- **Paths resolve against `/proc/self/exe`'s directory, never the cwd**,
  because `dlopen` does not search the cwd. That cost a debugging round when
  the core sat right beside the binary and still failed to load.
...
neither "emulation" nor "presentation" — it is the pixel pipeline between them.
Pixel-bound, confirmed by a scale sweep rather than assumed:
...
Boy Advance for less than a NES — against 14 to 23 ms of pixel pipeline. The v1
spec ruled SNES and GBA out of scope on CPU grounds and was wrong about both.

**Cost scales with DESTINATION pixels, not source pixels**, which is the trap
when comparing systems. A Game & Watch canvas is 654×396, seventeen times the
Game Boy's 160×144, suggesting a twenty-fold blowup. The opposite: G&W renders
at 1× (Parachute 260k px, 10.1 ms) while the Game Boy is upscaled to 800×720
(576k px, 16.6 ms). **G&W is cheaper than the Game Boy.** (`#30`)

`video_submit` is where the next optimisation belongs; v2's multi-rect work
optimised `refresh`, already the cheapest stage by a wide margin. (`#23`)
...
But `(R+G+B)/3` crushes **more** pixels to black than luma — 8.9% against 6.7%
over 38 gameplay frames from 19 titles — because what it gives back to blue it
takes from green. What fixes the crushing is a **shadow lift**, equivalent to
lowering the first quantiser threshold: 6.7% → 2.5%. The shipped `balanced` map
needs both: weights `(81,118,57)` *and* the lift. Selectable at runtime
(`MENU → GREYSCALE`, written back to the ini) because it is a judgement about a
reflective panel that cannot honestly be made from a host render.

**Two things not to do.** The Game Boy's palette is already neutral — gambatte
emits exactly `(0,0,0)`, `(82,85,82)`, `(173,170,173)`, `(255,255,255)`, every
mapping is the identity on neutral grey, and the lift keeps both mid greys
inside their levels. The DMG golden is byte-identical under all five maps, so
**no per-system exemption exists and none is needed**. And do not add one keyed
on 160×144: **a Game Gear is also 160×144 and is a colour system.**
`tests/test_video_gray.c` pins that, and the mutant that would have been the
bug (a `gray_map` exemption for 160×144 in `video_create`) makes it fail on
Sonic's measured sky. (`#46`)
...
The oldest open defect — moving sprites smearing — was fixed by the one thing
nobody had tried: rendering the game as **genuinely two-valued content**
instead of four grey levels.

A DU-class waveform drives a changed pixel to black or white. Ask for an
intermediate grey at partial-refresh speed and it lands between: stale ghost
above the sprite, overshoot *brighter than the background* below — both
directions failing at once, which is what the owner's photographs showed.
Sampled off the live framebuffer, the cause was in the data: Super Mario
Bros.'s sky is written at level 2, **a mid grey**, so every sprite transition
had to travel most of the way to an extreme.
...
**"DU4 cannot erase" is the same mechanism from the other side:** DU4 is the
four-level variant, and every pixel whose new value is one of the two *middle*
levels is one the panel declines to touch. The two findings never contradicted
each other.
...
where four-level content asks for nothing the panel cannot do.
...
no corner case; it is the Game Boy's lightest shade and most HUD text
everywhere else. Fixed by scaling the thresholds to 0..254 (`g_thresh`). Left
unfixed, "1-bit looks dirty" would have been true for a reason with nothing to
do with motion.
...
The newest and most consequential measurement in the project, and it overturned
a design assumption. `koboy-probe --coexist`, five region sizes spanning **49×
in area**, affine fit, reproduced to 0.1% on a re-run:
...
across that 49× span, and the per-pixel term is the same for every waveform
(15–22 ns) because it is the controller's pixel processing.

This does **not** make dirty rectangles a mistake — the area term is real, and
a smaller rect leaves less of the picture in flight. It means **a small update
is not a *fast* update**, the opposite of what the design assumed.
...
134 ms, and the probe says 153. Exactly the shape of "8 helped a lot and did
not finish the job".
...
obviously wrong once the settle model exists. `pacing.c` has **two gates,
ANDed**:
...
Read the pairs together. At divisor 3 the panel was asked for **more than
twice what it can finish** — one update every 64 ms against a 153 ms settle.
At divisor 8 the throttle is **very nearly inert: 90 → 89**, which is the
strongest evidence the model is right and nobody arranged it: divisor 8 is what
the owner reached *by eye*, and the model independently agrees there is nothing
left to take away. Divisor 3 with the throttle beats the manual 8 outright —
112 presented frames against 90 — because it gives back the small-area frames a
flat divisor throws away.

**Two load-bearing implementation notes.** `settle_base_ms` ships at **0**
although the measured fixed term is 144 ms, deliberately: the model decides
...
And the gate advances in `pacer_tick`, **not** `pacer_presented`: `main.c`
exits early on an unchanged frame, and a gate waiting on the presentation would
run `video_submit` — the 17 ms bottleneck — on every core frame of a static
screen.
...
limit of the INSTRUMENT. An on-device sweep found 20, 40 and 80 **behaviourally
identical on real content** — same rect count, same frame count — and splitting
is measurably cheaper to *submit* with the split off (368 µs against
488–750 µs), which is mechanical: each extra rect is another ioctl.

But refreshes are non-blocking by design, so the in-process timer measures
*submission*, not the panel's work — precisely the cost the setting exists to
amortise. Measuring the real benefit needs blocking refreshes, and this device
reports `unreliable_wait_for=1`, so those figures are suspect by construction.
(`#24`)
...
scale ceiling. A pure function of the ROM's name, called at load time, because
the browser hands `main.c` a path long after the config was read.

**The extension is the only signal, which constrains what can be claimed.**
Three deliberate refusals, each of which will be asked about:
...
So `main.c` polls `core_geometry_changed()` **every frame**, which also covers
...
`Donkey Kong Country` is the case the fallback exists for: it sends 512-wide
frames and **announces nothing** — `widths` says 256 and 512, `geom_calls` says
0. A frame between base and max, with no warning, into a rect sized from base.
`video_fit_rect` shrinks a frame the rect cannot hold; **do not delete that
branch.**
...
`KOBOY_LAYOUT_LCD` is a full-width **fractional** fit with a control strip,
because a Game & Watch unit *draws its own buttons* into the frame. SNES and
Mega Drive later moved to it too, their pads being A B X Y L R and A B C X Y Z
against the faceplate's two spare pockets — and the move was measured to cost
the two heaviest SNES titles nothing (identical picture size, identical
presented-frame counts, differences inside run-to-run spread).
...
**Two systems have a twelve-key keypad the faceplate cannot draw, and titles
on both refuse to start without it.** Proved by running: Donkey Kong on
ColecoVision sat on its option screen for 900 frames of START and A and started
the moment `JOYPAD_Y` — Gearcoleco's keypad 1 — was pressed. Intellivision's
route is stranger: FreeIntv puts keypad 1–9 only on the right analog stick,
which koboy has no source for, but also has a hold-to-show mini keypad on
`JOYPAD_L`, which makes **all twelve keys reachable**. Check any new system's
real control set against what the faceplate offers *before* assuming DMG is
enough; this has cost a round on four systems. (`#49`, `#50`)

**Arcade is the first system where "what does the hardware have" has no single
answer** — 227 boards — so the two discs were chosen by **counting every
romset's input descriptors**. FBNeo's map is flat (B = Button 1, A = 2, Y = 3,
X = 4, SELECT = Coin), and the counts decided it: Y on 134 boards, X on 71,
...
pixel and is the bottleneck on every system.

It starts with a fix. The reserved rect used to be sized from the core's
**max** geometry, which made "any frame in [1, max] fits" true by construction
— and cost a SNES 54% of its picture against a 512×512 mode snes9x2005 never
enters. Sizing from **base** instead:
...
**PC Engine got 2.25× the picture for free and slightly better** — its old rect
reserved 568k pixels to show 283k of picture, and the diff, blit and refresh
all run over the *rect*. **SNES is where the bill landed**, and only on the two
titles with no headroom:
...
Hence the ceilings. Every number is a device measurement against the 15,024 ms
that 900 frames of 59.9227 Hz content takes in real time, so **ideal ÷ measured
*is* the speed**:
...
pixels, all reported as slow in play — on frames *smaller than the Game Boy's
or the same size*. As pipeline capacity (`1000 / (submit + blit + refresh)`)
against a demand of 29.96 presented frames a second:
...
exemption". True when written. Then `pixel_aspect` shipped, the reserved rect
became 192 columns wide, the auto-fit went 5 → 6, and the picture became
1152×864 — 1.73× the area — **with nothing watching, because the reasoning
lived in a comment rather than a test.**

**The GBA ceiling is what makes the system exist.** A GBA frame is 240×160, the
smallest koboy scales, so it auto-fits furthest — uncapped it takes the LCD
strip's full width, 1264×842:
...
Most systems have square pixels; four do not. The correction earns itself
clearest on PC Engine, which changes horizontal resolution **mid-game** —
...
**The Atari 2600 forced this work, and how it was found is the point.** The
2600's pixels are ~1.6:1, and stella2014 says so the only way libretro lets it
— by declaring `base_width = 160 × 2` while delivering a 160-wide frame. koboy
scaled squarely, so **every 2600 title rendered about 1.75× too tall**:
Ms. Pac-Man's maze came out 480×630 where the correct shape is about 840×630.
Playable, legible, plainly wrong, and found by rendering frames and looking
*after every numeric check passed*. Fixed — BurgerTime is 840×630 and PAL
Breakout 1000×750, 4:3 to the pixel, pinned by `tests/test_video_aspect.c`.
Two things the diagnosis got wrong: the 2600 was **not** the only affected
system (eight are), and `base_width / delivered width` is the WRONG signal —
it gives 2.0 where the truth is 1.75. `aspect_ratio` is the right one. (`#65`)
...
Arcade is why this exists: FBNeo renders Galaga into a 288×224 *landscape*
buffer and asks for three quarter turns. Checked before assuming the core could
do it itself — its `fbneo-vertical-mode` option, whose name promises exactly
that, does **not** rotate the framebuffer: measured across all five settings,
the delivered frame stays 288×224 and only the aspect ratio and the
`SET_ROTATION` value move.
...
temp-file → `fsync` → `rename` out, **all-or-nothing read** in.

That read discipline is not theoretical: loading a truncated `.srm` used to
destroy it further. koboy now reports it, draws the message on the panel,
...
and, once running, "the index of the highest byte that is not `0xFF`, plus one"
— measured across a collection as anything from 285 bytes to 32160, and **0 for
a cartridge nobody has saved on**. So `core_sram` pins the length at ROM-load
time; without that, one `MENU → LOAD STATE` would rewrite a 65536-byte `.srm`
at 8191 bytes and the next launch would report it corrupt. Checked rather than
assumed for the others: snes9x2005's is constant, gpSP reports a flat 131072.
...
whatever `GET_SAVE_DIRECTORY` answers — observed directly, fourteen titles into
an empty directory left twelve files. An arcade board has no battery at all;
FBNeo's `hiscore.dat` is what exists instead, and koboy turns it on.
