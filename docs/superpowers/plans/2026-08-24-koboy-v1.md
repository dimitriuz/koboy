...
/* Variadic, not TEST_MAIN(body): braces do NOT protect commas in
   preprocessor arguments, so a body containing a top-level comma
   (e.g. `int a = 0, b = 1;`) would split into several arguments. */
                   ...
                   const koboy_layout *l)
{
    ...
    /* Bezel around the screen. Offset by one pixel and grown by two so the
       i==0 layer lands just OUTSIDE the game rect: frame() draws its first
       layer on the given border itself, so passing the rect directly would
       paint a 1px line into the picture area. */
    frame(fb, stride, p->game_x - 1, p->game_y - 1,
          p->game_w + 2, p->game_h + 2, 6, INK);
...
/* Install these immediately after platform init succeeds -- BEFORE the chrome
   draw and the calibration wait loop. Calibration blocks on a keypress and
   checks g_stop, so handlers registered later leave that check dead and a
   SIGTERM during calibration falls through to default terminate. */
...
while (!g_stop && !pf->should_quit(pf->ctx)) {
    ...
    /* A value <= 0 disables cleanup. The explicit guard is required: without
       it, 0 makes this always true (a full refresh every presented frame,
       the inverse of "never") and a negative value wraps the cast to a huge
       number so cleanup never runs at all. */
